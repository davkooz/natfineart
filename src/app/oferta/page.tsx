        ...
        <main className="bg-white min-w-screen min-h-screen flex flex-col">
            ...
            <div className="flex flex-wrap justify-center gap-8 px-4">
                {[
                    ...
                    <motion.div whileHover={{ y: 6, transition: { duration: 0.3 } }} key={index} className="flex flex-col items-center w-full md:w-96 border-2 border-black rounded-2xl gap-6 p-8 m-2">
                        <h1 className="font-bold text-3xl text-black text-center">{pakiet.title}</h1>
                        <hr className="border-black border-1 rounded-full w-32" />
                        <h2 className="font-bold text-2xl">{pakiet.price}</h2>
                        <ul className="list-disc space-y-1 text-left">
                            {pakiet.items.map((item, itemIndex) => (
                                <li key={itemIndex} className="text-xl">{item}</li>
                            ))}
                        </ul>
